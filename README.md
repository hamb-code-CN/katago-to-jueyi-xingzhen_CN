...
- ⚖️ **打劫（ko）处理**：逐帧跟踪真实落子历史并同步给 KataGo，劫争中主动找劫材；本地二次合法性校验 + 防死循环
- 🖥️ **实时看板**（http://127.0.0.1:8123/analysis.html）：
...
- 🗂️ **双平台自适应**：窗口拖动、缩放、最小化（腾讯走 PrintWindow）均能定位
...
启动后 AI 自动对局。**建议从一局新棋开局**（AI 从空盘记录落子历史，打劫处理最完整）。
...
**Q：为什么偶尔出现"历史不一致, 清空重来"日志？**
A：AI 从中途（非空盘）接入时没有完整落子历史，无法 100% 还原劫状态，属正常降级 —— 但防死循环保护始终生效。**从新局开局即可完整打劫**。
...
每个循环:
  ...
  ⑤ 合法性校验(含劫): 非法点重决策, 4 轮仍非法则放弃本轮(防死循环)
